...
- Runtime exports are exactly `name`, `inject`, and `apply`; declarations expose the same API. Host SDK imports stay
  external as `"*"` peers while build-time declarations stay pinned to the supported floor. Harness resolves those
  peers through its profile loader; making `dsh-tools` a local dependency risks a duplicate scheduler-symbol realm.
